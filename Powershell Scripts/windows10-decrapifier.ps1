...
#
#Read the how to for easy steps to run this script:
...
#
#
#BEST RESULTS: Run this in the audit stage, either during image creation from MDT etc, or by pushing CTRL+SHIFT+F3 on the OOBE screen. Save headaches messing with settings and apps, and get (some) functionality back that you used to have with GPOs that no longer work for Pro editions.
#
#				OR
#
#			   Run this on a brand new machine, first thing after initial login. 
#              Note it will not clear the start menu or set certain settings for the user account it is run from. Running from audit mode will not have this issue.
#
#
